...
def get_log_dir(args: argparse.Namespace, scenario: ScenarioCfg) -> str:
    ...
    if hasattr(args, "run_name") and args.run_name is not None:
        ...
        # Create timestamp as fallback
        from datetime import datetime, timedelta, timezone

        china_tz = timezone(timedelta(hours=8))
        run_name = datetime.now(china_tz).strftime("%Y_%m%d_%H%M%S")
...
def get_load_path(args: argparse.Namespace, scenario: ScenarioCfg) -> str:
    ...
    if args.checkpoint == -1:
        models = [file for file in os.listdir(load_root) if file.startswith("model_") and file.endswith(".pt")]
        models.sort(key=lambda m: int(m.split("_")[1].split(".")[0]))  # 按数字排序
        model = models[-1]
        load_path = os.path.join(load_root, model)
    else:
        load_path = os.path.join(load_root, f"model_{args.checkpoint}.pt")
    return load_path
...
def get_args(test=False):
    ...
    custom_parameters = [
        ...
        },
        {
            ...
            "type": str,
            "default": None,
            "help": "Name of the run to load/resume from. Required when using --resume flag.",
        },
        {
            "name": "--checkpoint",
            "type": int,
        ...
        {"name": "--wandb", "type": str, "default": "h1_skillblender_roboverse", "help": "Wandb project name"},
        {"name": "--entity", "type": str, "default": None, "help": "Wandb entity name"},
    ]
